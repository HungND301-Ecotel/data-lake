...
const DataComponent: React.FC<DataProps> = ({
  ...
  const [groupAdvance, setGroupAdvance] = useState<string>(
    data.groupAdvance ?? ""
  );
  const [selectAdvance, setSelectAdvance] = useState<string>(
    data.selectAdvance ?? ""
  );
  ...
  >({
    ...
    selectAdvance: data.selectAdvance,
  });
  ...
  return (
    <Tabs defaultActiveKey="1" type="card">
      ...
      {isEditMode && (
        <TabPane tab="Nâng cao" key="7">
          <div className="p-2 border rounded bg-gray-50 space-y-4">
            <div>
              ...
              <Input.TextArea
                rows={3}
                value={selectAdvance}
                onChange={(e) => setSelectAdvance(e.target.value)}
                placeholder="VD: SUM(amount) AS total_amount"
              />
            </div>
            <div>
              <label className="block mb-1 font-medium">
                Group By nâng cao (SQL):
              </label>
              <Input.TextArea
                rows={3}
                value={groupAdvance}
                onChange={(e) => setGroupAdvance(e.target.value)}
                placeholder="VD: GROUP BY YEAR(created_at), department_id"
              />
            </div>
          </div>
        </TabPane>
