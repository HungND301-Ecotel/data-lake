...
export default function DashboardPage() {
  ...
  const filteredProduction = PRODUCTION_TABLE.map((row, index) => {
    const rand = seedRandom(seed + index);
    const factor = 0.85 + rand * 0.3;
    let homNay = row.homNay * factor;
    let luyKe = row.luyKe * factor * periodMultiplier;
    let kh = row.kh * periodMultiplier;

    // Apply department-specific filtering
    let match = true;
    if (deptFilter !== "Tất cả") {
      if (deptFilter === "PX Than Nguyên Khai") {
        match = row.chiTieu === "Than NK sản xuất";
      } else if (deptFilter === "PX Than Sạch") {
        match = row.chiTieu === "Than NK sản xuất";
      } else if (deptFilter === "PX Cơ điện") {
        match = ["CBSX", "Mét lò đào mới"].includes(row.chiTieu);
      } else {
        match = false; // Office depts don't produce coal/excavation
      }
    }

    return {
      ...row,
      homNay: match ? Math.round(homNay) : 0,
      luyKe: match ? Math.round(luyKe * 10) / 10 : 0,
      kh: match ? Math.round(kh) : 0,
    };
  });

  const filteredWorkers = WORKER_TABLE.map((row, index) => {
    const rand = seedRandom(seed + index + 10);
    const factor = 0.8 + rand * 0.4;

    let o = typeof row.o === "number" ? Math.round(row.o * factor) : row.o;
    let tLo =
      typeof row.tLo === "number" ? Math.round(row.tLo * factor) : row.tLo;
    let dien =
      typeof row.dien === "number" ? Math.round(row.dien * factor) : row.dien;
    let co = typeof row.co === "number" ? Math.round(row.co * factor) : row.co;
    let bch =
      typeof row.bch === "number" ? Math.round(row.bch * factor) : row.bch;
    let tLoVang =
      typeof row.tLoVang === "number"
        ? Math.round(row.tLoVang * factor)
        : row.tLoVang;
    let cdienVang =
      typeof row.cdienVang === "number"
        ? Math.round(row.cdienVang * factor)
        : row.cdienVang;

    let match = true;
    if (deptFilter !== "Tất cả") {
      if (deptFilter === "PX Than Nguyên Khai") {
        match = ["Thợ lò", "Phó QĐ trực ca"].includes(row.nhanluc);
      } else if (deptFilter === "PX Than Sạch") {
        match = ["Thợ lò", "Phó QĐ trực ca"].includes(row.nhanluc);
      } else if (deptFilter === "PX Cơ điện") {
        match = ["Cơ điện", "Phó QĐ trực ca"].includes(row.nhanluc);
      } else if (
        ["Phòng Kế hoạch - Vật tư", "Phòng Kế toán", "Phòng Nhân sự"].includes(
          deptFilter,
        )
      ) {
        match = ["BCH, Phục vụ"].includes(row.nhanluc);
      }
    }
    ...
    return {
      ...
      co: match ? co : "",
      bch: match ? bch : "",
      tLoVang: match ? tLoVang : "",
      cdienVang: match ? cdienVang : "",
    };
  });
  ...
  // ─── NEW SYSTEM METRICS FROM OLD DASHBOARD ──────────────────────────────
  const reportsCountVal = Math.round(
    (1128 + seedRandom(seed + 30) * 100) * periodMultiplier,
  );
  const departmentsCountVal = departments.length - 1;
  const usersCountVal = Math.round(58 + seedRandom(seed + 31) * 12);
  ...
  return (
    <>
      <div className="bg-[#f8faf9] min-h-screen p-8">
        <div className="max-w-[1400px] mx-auto flex flex-col gap-7">
          {/* ═══ HEADER ═══ */}
          <div className="bg-white rounded-2xl p-7 flex justify-between items-center shadow-[0_4px_20px_-2px_rgba(15,23,42,0.03)] border border-slate-100 relative overflow-hidden before:content-[''] before:absolute ">
          ...
          {/* ═══ FILTER BAR ═══ */}
          <div className="bg-white rounded-xl p-4 flex gap-6 items-center flex-wrap shadow-sm border border-slate-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                type="date"
                className="border border-slate-100 bg-slate-50 rounded-lg px-3 py-2 text-xs text-slate-900 outline-none cursor-pointer transition-all hover:border-slate-300 focus:border-[#1a8649] focus:bg-white"
                value={selectedDate}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.03)] overflow-hidden flex flex-col">
              <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <span className="font-bold text-base lg:text-xl text-slate-900 tracking-tight">
                  Bảng công nhân lực chi tiết
                </span>
                <span className="text-xs lg:text-sm font-semibold px-3 py-1 rounded-md bg-emerald-50 text-emerald-600 inline-flex items-center gap-1">
                  Live Telemetry
                </span>
              </div>
                ...
                <table className="w-full border-collapse text-left">
                  <thead>
                    <tr className="border-b border-slate-100">
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider">
                        Nhân lực
                      </th>
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center">
                        Ô
                      </th>
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center">
                        F
                      ...
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center bg-slate-50 rounded-l-lg">
                        T.Lò
                      </th>
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center bg-slate-50">
                        Điện
                      </th>
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center bg-slate-50">
                        Cơ
                      </th>
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center bg-slate-50 rounded-r-lg">
                        BCH
                      </th>
                      <th className="p-4.5 px-4 text-xs lg:text-sm font-bold uppercase text-slate-500 tracking-wider text-center">
                        T.Lò vắng
                  ...
                  <tbody>
                    {filteredWorkers.map((row, i) => {
                      ...
                      return (
                        ...
                        >
                          <td className="p-5 px-4 text-sm lg:text-base text-slate-900 font-semibold">
                            <span
                              className={`text-xs lg:text-sm font-semibold px-3 py-1 rounded-md inline-flex items-center gap-1 ${badgeClass}`}
                            >
                              {row.nhanluc}
                            </span>
                          </td>
                          <td className="p-5 px-4 text-sm lg:text-base text-slate-900 text-center">
                            {row.o}
                          </td>
                          <td
                            className="p-5 px-4 text-sm lg:text-base text-center"
                            style={{
                              fontWeight: row.f === 0 ? 700 : 400,
                              color: row.f === 0 ? "#1a8649" : "inherit",
                            }}
                          >
                            {row.f}
                          </td>
                          ...
                          <td className="p-5 px-4 text-sm lg:text-base text-center bg-slate-50/50 font-bold">
                            {row.tLo}
                          </td>
                          <td className="p-5 px-4 text-sm lg:text-base text-center bg-slate-50/50 font-bold">
                            {row.dien}
                          ...
                          <td
                            className="p-5 px-4 text-sm lg:text-base text-center"
                            style={{
                              color: row.tLoVang ? "#ef4444" : "inherit",
                              fontWeight: row.tLoVang ? 700 : 400,
                            }}
                          >
                            {row.tLoVang}
                          </td>
                          <td
                            ...
                            style={{
                              color:
                                row.cdienVang === 0 ? "#10b981" : "inherit",
                              fontWeight: row.cdienVang === 0 ? 700 : 400,
                            }}
                          >
